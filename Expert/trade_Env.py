...
class portfolio_tradeEnv:
    ...
    def step(self, action):
        ...
        if self.terminal:
            ...
            self.balance = end_assert_value
            self.shares.append(0)
            self.rate.append(np.log(1+(self.balance - begin_assert_value) / begin_assert_value))
    ...
    def buy(self, action):
        ...
        if self.balance > 0:
            self.shares.append((1 - self.transaction_cost) * self.balance / self.stock_state.Open.values[-1])
            self.balance = 0
    ...
    def sell(self, action):
        # 全部卖出
        cash = self.stock_state.Open.values[-1] * self.shares[-1] * (1 - self.transaction_cost)
        self.balance += cash
        # 更改份额
        self.shares.append(0)
